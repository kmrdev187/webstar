...
export class EditCharacterComponent {
	...
	validate() {
		...
		this.selectedDescription == '' &&
			(this.validationError = 'A leírás mező nem lehet üres!');
		typeof this.selectedMidichlorian !== 'number' &&
			(this.validationError = 'Hibás midiklorián érték!');
		this.selectedPower == '' &&
			(this.validationError = 'Válassz egy különleges erőt!');
		this.selectedSide == '' &&
			(this.validationError = 'Válassz egy oldalt!');
		(this.selectedName == '' || this.selectedName.trim().length === 0) &&
			(this.validationError = 'A név mező nem lehet üres!');
		this.selectedAvatar == '' &&
			(this.validationError = 'Válassz egy avatárt!');
